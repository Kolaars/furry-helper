...
function main()
	...
	lua_thread.create(function()
		...
		local json_str = encode_table(payload)
		-- ?????????? ??????? ??? ????????? ??????
		json_str = json_str:gsub('"', '\\"')
		
		local cmd = 'curl -X POST -H "Content-Type: application/json" -d "' .. json_str .. '" "' .. webhook_url .. '"'
		
		os.execute(cmd)
		sampAddChatMessage("[Furry Helper] {00ff00}?????????? ????? curl", 0x00FF00)
	end)
